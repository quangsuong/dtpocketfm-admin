...
<body>
    ...
    <script>
        ...
        $('#imageUploadLandscapeModel').change(function() {
         ...
         // Image Upload Preview Add (Web Banner)
         $('#imageUploadWebBanner').change(function() {
            if (this.files && this.files[0]) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $('#imagePreviewWebBanner').attr("src", e.target.result);
                    $('#imagePreviewWebBanner').hide();
                    $('#imagePreviewWebBanner').fadeIn(650);
                }
                reader.readAsDataURL(this.files[0]);
            }
        });
        // Image Upload Preview Add Model (Landscape)
        $('#imageUploadWebBannerModel').change(function() {
            if (this.files && this.files[0]) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    $('#imagePreviewWebBannerModel').attr("src", e.target.result);
                    $('#imagePreviewWebBannerModel').hide();
                    $('#imagePreviewWebBannerModel').fadeIn(650);
                }
                reader.readAsDataURL(this.files[0]);
            }
        });
