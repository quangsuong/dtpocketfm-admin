...
class AvatarController extends Controller
{
            ...
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        $delete = ' <form onsubmit="return confirm(\'Are you sure !!! You want to Delete this Avatar ?\');" method="POST"  action="' . route('avatar.destroy', [$row->id]) . '">
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('avatar.index')->with('success', __('Label.data_delete_successfully'));
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
